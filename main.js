...
var Pic2Tex = class {
  ...
  async sendRequest(imageBuffer) {
    const formData = new FormData();
    formData.append("image", new Blob([imageBuffer]), "formula.png");
    const response = await fetch(this.settings.pix2tex.url, {
      method: "POST",
      body: formData
    });
...
var DEFAULT_SETTINGS = {
  ...
  aiModel: {
    ...
    maxTokens: 300,
    prompts: {
      latex: "Convert the math equation in the image to LaTeX format.",
      markdown: "Convert the math equation in the image to Markdown format."
    }
  }
};
var OCRLatexSettings = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    const readmeURL = "https://github.com/RandyAllenEEE/obsidian-ocrlatex/new/master/README.md";
    ...
    new import_obsidian.Setting(containerEl).setName("Markdown provider").setDesc("Choose which provider to use for Markdown conversion").addDropdown((dropdown) => {
      dropdown.addOptions({
        "Texify": "Texify",
        "LLM": "LLM"
      }).setValue(this.plugin.settings.markdownProvider).onChange(async (value) => {
        this.plugin.settings.markdownProvider = value;
        await this.plugin.saveSettings();
      });
    });
    ...
    new import_obsidian.Setting(containerEl).setName("Endpoint").setDesc("LLM endpoint URL (e.g., https://api.openai.com/v1/chat/completions)").addText((text) => {
      text.setValue(this.plugin.settings.aiModel.endpoint).onChange(async (value) => {
        this.plugin.settings.aiModel.endpoint = value;
        await this.plugin.saveSettings();
      });
    });
    
    new import_obsidian.Setting(containerEl).setName("Model").setDesc("LLM model name (e.g., gpt-4-vision-preview)").addText((text) => {
      text.setValue(this.plugin.settings.aiModel.model).onChange(async (value) => {
        this.plugin.settings.aiModel.model = value;
        await this.plugin.saveSettings();
      });
    });
    
    new import_obsidian.Setting(containerEl).setName("API Key").setDesc("API key for authentication").addText((text) => {
      text.setValue(this.plugin.settings.aiModel.apiKey).onChange(async (value) => {
        this.plugin.settings.aiModel.apiKey = value;
        await this.plugin.saveSettings();
      });
    });
    
    new import_obsidian.Setting(containerEl).setName("Max Tokens").setDesc("Maximum number of tokens to generate").addText((text) => {
      text.setValue(this.plugin.settings.aiModel.maxTokens.toString()).onChange(async (value) => {
        const numValue = parseInt(value);
        if (!isNaN(numValue) && numValue > 0) {
          this.plugin.settings.aiModel.maxTokens = numValue;
          await this.plugin.saveSettings();
        }
      });
    });
    ...
    containerEl.createEl("h3", { text: "LLM Prompts" });
    
    new import_obsidian.Setting(containerEl).setName("LaTeX Prompt").setDesc("Prompt for LaTeX conversion (used for both inline and multiline)").addTextArea((text) => {
      text.setValue(this.plugin.settings.aiModel.prompts.latex).onChange(async (value) => {
        this.plugin.settings.aiModel.prompts.latex = value;
        await this.plugin.saveSettings();
      });
    }).setClass("llm-prompt-setting");
    
    new import_obsidian.Setting(containerEl).setName("Markdown Prompt").setDesc("Prompt for markdown conversion").addTextArea((text) => {
      text.setValue(this.plugin.settings.aiModel.prompts.markdown).onChange(async (value) => {
        this.plugin.settings.aiModel.prompts.markdown = value;
        await this.plugin.saveSettings();
      });
    }).setClass("llm-prompt-setting");
  }
...
function getLatexProvider(isMultiline, settings) {
  ...
  } else if (settings.latexProvider === "Pix2Tex") {
...
function getMarkdownProvider(settings) {
  if (settings.markdownProvider === "Texify") {
    return new Texify(settings.texify);
  } else {
...
var OCRLatexPlugin = class extends import_obsidian2.Plugin {
  ...
  async onload() {
    ...
    this.addCommand({
      ...
      name: "Generate markdown from last image to clipboard",
      callback: async () => {
        this.insert(getMarkdownProvider(this.settings));
      }
    });
  ...
  async loadSettings() {
    ...
    if (!this.settings.aiModel) {
      this.settings.aiModel = DEFAULT_SETTINGS.aiModel;
    } else {
      // 确保 aiModel 包含所有必要的字段
      if (!this.settings.aiModel.prompts) {
        this.settings.aiModel.prompts = DEFAULT_SETTINGS.aiModel.prompts;
      } else {
        // 确保 prompts 包含所有必要的字段
        if (this.settings.aiModel.prompts.latex === undefined) {
          this.settings.aiModel.prompts.latex = DEFAULT_SETTINGS.aiModel.prompts.latex;
        }
        if (this.settings.aiModel.prompts.markdown === undefined) {
          this.settings.aiModel.prompts.markdown = DEFAULT_SETTINGS.aiModel.prompts.markdown;
        }
      }
      
      if (this.settings.aiModel.maxTokens === undefined) {
        this.settings.aiModel.maxTokens = DEFAULT_SETTINGS.aiModel.maxTokens;
      }
    }
  }
  async saveSettings() {
